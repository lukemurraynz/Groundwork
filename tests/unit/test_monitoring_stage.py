"""T082 — the ``monitoring`` stage.

**Rewritten 2026-08-24 (Clarifications, FR-038a)**: no longer writes diagnostic settings directly
(System's credential cannot touch the customer subscription past bootstrap, and Fabric capacity
provisioning has no pipeline coverage yet regardless — see module docstring). Now
verification-only against the same Azure DevOps pipeline run ``infrastructure`` triggered —
identical shape to ``test_networking_stage.py``/``test_identity_stage.py``.
"""
...
from groundwork_orchestrator.stages.monitoring import MonitoringStage, MonitoringStageError
...
async def test_raises_when_no_run_has_ever_been_triggered(valid_plan) -> None:
    ...
    with pytest.raises(MonitoringStageError, match="no pipeline run"):
...
async def test_raises_without_a_devops_organization_url(valid_plan) -> None:
    ...
    stage = MonitoringStage(http_client=http_client)
    context = _context(plan=valid_plan, organization_url=None)

    with pytest.raises(MonitoringStageError, match="organization"):
        await stage.execute(context)
